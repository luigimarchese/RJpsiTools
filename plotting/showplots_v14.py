                        ...
                        if 'data' in name:
                            ...
                            for sname in sample_names:
                                if sname in name:
                                    #print(sname,name)
                                    sample = sname
                                    shapes[name] = shapes[name].Define('shape_weight_wfr_norm','shape_weight_tmp*fakerate_bcmu*%f/%f'%(mean_nn_data_weights_lm[sample],mean_nn_mc_weights_lm[sample])) #fail region
                                    break
                    else:
        ...
        for k, v in histos.items():
            ...
            for sample,sample_item in samples.items():
                if "jpsi_x_mu" in sample:
                    if not jpsi_x_mu_split: # The general binbybin only for jpsi_x_mu when it is not splitted
