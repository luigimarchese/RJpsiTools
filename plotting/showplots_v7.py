...
from plot_shape_nuisances_v2 import plot_shape_nuisances
...
def define_shape_nuisances(sname, shapes, samples, nuisance_name, central_value, up_value, down_value, central_weights_string):
    ...
    if sname == 'jpsi_mu':
        shapes[sname + '_' + nuisance_name + 'Up'] = shapes[sname + '_' + nuisance_name + 'Up'].Define('shape_weight_tmp', central_weights_string.replace(central_value,up_value)+'*hammer_bglvar*%f'%(ff_weights[sname]))
    elif sname == 'jpsi_tau':
        shapes[sname + '_' + nuisance_name + 'Up'] = shapes[sname + '_' + nuisance_name + 'Up'].Define('shape_weight_tmp', central_weights_string.replace(central_value,up_value)+'*hammer_bglvar*%f*%f*%f' %(blind,rjpsi,ff_weights[sname]))
    ...
    if sname == 'jpsi_mu':
        shapes[sname + '_' + nuisance_name + 'Down'] = shapes[sname + '_' + nuisance_name + 'Down'].Define('shape_weight_tmp', central_weights_string.replace(central_value,down_value)+'*hammer_bglvar*%f'%(ff_weights[sname]))
    elif sname == 'jpsi_tau':
        shapes[sname + '_' + nuisance_name + 'Down'] = shapes[sname + '_' + nuisance_name + 'Down'].Define('shape_weight_tmp', central_weights_string.replace(central_value,down_value)+'*hammer_bglvar*%f*%f*%f' %(blind,rjpsi,ff_weights[sname]))
...
if __name__ == '__main__':
    ...
    for k, v in samples_orig.items():
        ...
        if k=='jpsi_tau':
            samples_orig[k] = samples_orig[k].Define('tmp_weight', central_weights_string +'*hammer_bglvar*%f*%f*%f' %(blind,rjpsi,ff_weights[k]))
        elif k=='jpsi_mu':
            samples_orig[k] = samples_orig[k].Define('tmp_weight', central_weights_string +'*hammer_bglvar*%f'%ff_weights[k])
        elif 'jpsi_x_mu' in k:
    ...
    for k, v in samples_orig.items():
        ...
        print("Sample "+k +" with "+str(len(samples_lm[k].AsNumpy()['nPV']))+" events")
   ...
   #Apply preselection for ch3 and ch4 (high mass region)
    if add_hm_categories:
        ...
        for k, v in samples_orig.items():
            ...
            print("Sample "+k +" with "+str(len(samples_hm[k].AsNumpy()['nPV']))+" events")
        ...
        for iter,(shapes,samples) in enumerate(zip(shapes_dictionaries, samples_dictionaries)):
            ...
            for sname in samples:
                ...
                if compute_sf_onlynorm:
                    ...
                    # Only jpsi for now, bc the sf_id for the third muon is only in the pass region!
                    if (sname != 'data'):
                        shapes[sname + '_sfIdUp'], shapes[sname + '_sfIdDown'] = define_shape_nuisances(sname, shapes, samples, 'sfId', 'sf_id_jpsi', 'sf_id_all_jpsi_up', 'sf_id_all_jpsi_down', central_weights_string)
            ...
            if not iter: #iter == 0
                ...
                for ham in hammer_branches:
                    ...
                    shapes['jpsi_mu_'+new_name] = shapes['jpsi_mu_'+new_name].Define('shape_weight_tmp', central_weights_string+'*'+ham+'*%f'%(ff_weights['jpsi_mu']))
                    shapes['jpsi_tau_'+new_name] = samples['jpsi_tau']
                    shapes['jpsi_tau_'+new_name] = shapes['jpsi_tau_'+new_name].Define('shape_weight_tmp', central_weights_string+'*'+ham+'*%f*%f*%f' %(blind,rjpsi,ff_weights['jpsi_tau']))
    ...
    for iteration,(shapes,samples,histos) in enumerate(zip(shapes_dictionaries, samples_dictionaries, histos_dictionaries)):
        ...
        for k, v in histos.items():
            ...
            if shape_nuisances and ((k in datacards and  iteration==0) or (k == 'Bmass' and iteration)):
                ...
                plot_shape_nuisances(label, k, channels[0], [name for name,v in samples.items()], compute_sf = compute_sf, compute_sf_onlynorm = compute_sf_onlynorm)
            #####################################################
            ...
            if shape_nuisances and ((k in datacards and  iteration==0) or (k == 'Bmass' and iteration)):
                ...
                if not only_pass:
                    plot_shape_nuisances(label, k, channels[1], [name for name,v in samples.items()], compute_sf = compute_sf, compute_sf_onlynorm = compute_sf_onlynorm)
