...
def norm_nuisances(f, channel, histos, jpsi_split):
    ...
    fakesmethod_string = 'fakesmethod shape '
    ...
    for i,histo in enumerate(histos):
        if histo == 'data':
            ...
            pu_string      += ' - '
            fakesshape_string   += ' 1 '
            fakesmethod_string   += ' 1 '
            fakes_string   += ' 1.1 '
            fakes_lm_string   += ' 1.05 '
            fakes_hm_string   += ' 1.05 '
            trigger_string += ' - '
...
def sf_nuisances(f, channel, histos, sfReco_nuisances, sfIdjpsi_nuisances, sfIdk_nuisances,jpsi_split):
    ...
    for histo in histos:
        if histo == 'data':
            ...
            string_reco += ' %.3f '%sfReco_nuisances[histo]
            if channel == 'ch1' or channel == 'ch3':
                string_iso_k += ' 1.1 '
            else:
                string_iso_k += ' 0.9 '
...
def create_datacard_ch1(label, var_name,  histos, hmlm_split, jpsi_x_mu_samples, which_sample_bbb_unc =[], add_dimuon = False, sf_reco ={}):
    ...
    sf_nuisances(f, 'ch1', histos, sf_reco, sfIdjpsi_nuisances, sfIdk_nuisances, jpsi_split)
...
def create_datacard_ch2(label, var_name,  histos, hmlm_split, jpsi_x_mu_samples, which_sample_bbb_unc =[], sf_reco={}):
    jpsi_split = len(jpsi_x_mu_samples)>1
    ...
    sf_nuisances(f, 'ch2', histos, sf_reco, sfIdjpsi_nuisances,sfIdk_nuisances, jpsi_split)
...
def create_datacard_ch3(label, var_name,  histos, hmlm_split, jpsi_x_mu_samples, which_sample_bbb_unc =[],add_dimuon = False, sf_reco={}):
    jpsi_split = len(jpsi_x_mu_samples)>1
    ...
    sf_nuisances(f, 'ch3', histos, sf_reco, sfIdjpsi_nuisances, sfIdk_nuisances,jpsi_split)
    if len(which_sample_bbb_unc)<1:
...
def create_datacard_ch4(label, var_name,  histos, hmlm_split, jpsi_x_mu_samples, which_sample_bbb_unc =[], sf_reco={}):
    jpsi_split = len(jpsi_x_mu_samples)>1
    ...
    sf_nuisances(f, 'ch4', histos, sf_reco, sfIdjpsi_nuisances, sfIdk_nuisances, jpsi_split)
    if len(which_sample_bbb_unc)<1:
