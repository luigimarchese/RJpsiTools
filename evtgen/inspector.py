...
#         tofill['ctau_weight_down_lhe'   ] = weight_to_new_ctau(ctaus[0], ctau_down, ct*10.)
        ...
        if abs(first_ancestor.pdgId()) in [521, 523]:
            final_daus = []
            if abs(jpsi.mother(0).pdgId())==521:
                the_b = jpsi.mother(0)
                its_a_b = True
            else:    
            ...
            tofill['is_jpsi_hc'  ] = ((final_daus==[431, 443]) or (final_daus==[433, 443])) 
...
#                 import pdb ; pdb.set_trace()
        ...
        if len(final_state_muons)>=3:
            ...
            tofill['m2_miss'    ] = (b_scaled_p4 - extra_mu.p4() - jpsi_muons[0].p4() - jpsi_muons[1].p4()).mass2()
            tofill['pt_miss_sca'] = b_scaled_p4.pt() - extra_mu.pt() - jpsi_muons[0].pt() - jpsi_muons[1].pt()
            tofill['pt_miss_vec'] = (b_scaled_p4 - extra_mu.p4() - jpsi_muons[0].p4() - jpsi_muons[1].p4()).pt()
            tofill['q2'         ] = (b_scaled_p4- jpsi.p4()).mass2()
            # tofill['e_star_mu3' ] = extra_mu.p4().E() * np.cosh(extra_mu.p4().Rapidity() - b_scaled_p4.Rapidity())
            # tofill['e_hash_mu3' ] = extra_mu.p4().E() * np.cosh(extra_mu.p4().Rapidity() - jpsi.p4().Rapidity())
